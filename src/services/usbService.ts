...
// USB Service with ESP32, Nucleo, and ShamanLink detection
...
interface KnownDevice {
  name: string;
  type: 'esp32' | 'nucleo' | 'shamanlink' | 'generic';
  description: string;
}
...
const KNOWN_DEVICES: { [key: string]: KnownDevice } = {
  // ===== ShamanLink =====
  '1fc9:0094': { name: 'ShamanLink', type: 'shamanlink', description: 'ShamanLink MTL Bridge (NXP 1FC9:0094)' },
...
const VENDOR_NAMES: { [key: string]: { name: string, type: 'esp32' | 'nucleo' | 'shamanlink' | 'generic' } } = {
  '303a': { name: 'Espressif',          type: 'esp32' },
  '10c4': { name: 'Silicon Labs',       type: 'esp32' },
  '1a86': { name: 'WCH',               type: 'esp32' },
  '0403': { name: 'FTDI',              type: 'generic' },
  '0483': { name: 'STMicroelectronics', type: 'nucleo' },
  '1fc9': { name: 'NXP / ShamanLink',  type: 'shamanlink' },
};
...
class UnifiedUSBService {
  ...
  isSupported(): boolean {
    if (platformService.isElectron()) return true;
  ...
  async requestPermission(): Promise<boolean> {
    ...
    try {
      ...
      this.notifyListeners({ action: 'connected', totem, timestamp: new Date() });
      return true;
    } catch (error) {
  ...
  async startMonitoring(): Promise<void> {
    ...
    this.scanInterval = window.setInterval(() => { this.scanDevices(); }, 2000);
  ...
  stopMonitoring(): void {
    ...
    this.connectedTotems.forEach((totem) => {
      if (!this.claimedPorts.has(totem.id)) platformService.closeSerialPort(totem.id);
    });
  ...
  private identifyDeviceByUSB(info: any, port: any, deviceId?: string): TotemStatus {
    ...
    // Map device type to totem type
    const totemType = deviceInfo.type === 'shamanlink' ? 'comm-uart' :
                      deviceInfo.type === 'esp32'      ? 'mcu-controller' :
    ...
    return {
      ...
      hardwareFault:    'none',
      capabilities: {
        hasADC:   deviceInfo.type !== 'shamanlink',
        hasPWM:   deviceInfo.type !== 'shamanlink',
        hasUART:  true,
        hasI2C:   true,
  ...
  /** Returns true if the given totem is a ShamanLink bridge */
  isShamanLink(totem: TotemStatus): boolean {
    const serial = totem.serialNumber?.toLowerCase() || '';
    return serial === '1fc9:0094' || totem.name === 'ShamanLink';
  }

  /** Returns the first connected ShamanLink, or null */
  getConnectedShamanLink(): TotemStatus | null {
    for (const totem of this.connectedTotems.values()) {
      if (this.isShamanLink(totem)) return totem;
    }
    return null;
  }
  ...
  private notifyListeners(event: USBConnectionEvent): void {
    this.listeners.forEach(cb => {
      try { cb(event); } catch (error) { console.error('Listener error:', error); }
    });
  }
  ...
  getDeviceType(totem: TotemStatus): 'esp32' | 'nucleo' | 'shamanlink' | 'unknown' {
    ...
    if (name.includes('esp32') || name.includes('espressif') ||
        ...
        serial.startsWith('303a') || serial.startsWith('10c4') || serial.startsWith('1a86')) {
      return 'esp32';
    }
